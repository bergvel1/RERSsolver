(* Dept-first search over control flow graphs, to be used with a blackboxed SMT solver *)
...
let rec smt_reverse_dfs graph task_stack =
if (Stack.is_empty task_stack) then print_string "no more tasks\n" else
	...
	if ((curr_task.depth > -1) && (curr_task.condition != False)) then
		...
		(match (get_in_nodes curr_node) with
		  [] -> Solver.assume curr_task.condition; Solver.check(); smt_reverse_dfs graph task_stack
		| in_edges -> let _ = List.map (fun e -> (match e with
			...
			| (n,_) -> Stack.push 
					{task_num = if (e = (List.hd (List.rev in_edges))) then curr_task.task_num else get_task_count();
					 ...
					 depth = curr_task.depth;
					 condition = curr_task.condition;
					 input_count = curr_task.input_count} task_stack;)) in_edges in smt_reverse_dfs graph task_stack))
	else (*print_string ("Terminated " ^ (string_of_task curr_task));*)
		smt_reverse_dfs graph  task_stack 

let init_smt_reverse_dfs graph depth start_node_idx =
	Solver.reset();
	reset_task_count(); 
